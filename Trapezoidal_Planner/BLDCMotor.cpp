...
void BLDCMotor::move(float new_target) {
  ...
  switch (controller) {
    ...
    case MotionControlType::angle:
      ...
      error = shaft_angle_sp - shaft_angle;
      // calculate velocity set point
      shaft_velocity_sp = P_angle( error ) + feed_forward_velocity;
      //if(fabs(shaft_velocity_sp)<= 0.2) shaft_velocity_sp = 0.0;
      //if(fabs(error) <= 0.1) error = 0;
      shaft_velocity_sp = _constrain(shaft_velocity_sp,-velocity_limit, velocity_limit);
      vel_err = shaft_velocity_sp - shaft_velocity;
      // calculate the torque command - sensor precision: this calculation is ok, but based on bad value from previous calculation
      //if(fabs(vel_err) <= 0.2) vel_err = 0.0;
      current_sp = PID_velocity(vel_err); // if voltage torque control
      //if(fabs(current_sp) <= 0.3) current_sp = 0.0;
